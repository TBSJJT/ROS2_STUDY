...
class BridgeConfig:
    ...
    # =========================================================================
    # port: 串口设备文件路径
    # Linux 下常见的命名: /dev/ttyUSB0, /dev/ttyACM0
    # 如果使用 udev 规则固定名称, 可能是 /dev/gasrobot_base
    port: str = "/dev/ttyUSB0"

    # baud: 串口通信波特率 (bits per second)
    # 115200 是常用的高速串口波特率
    # 必须与 STM32 固件中的波特率设置一致, 否则无法通信
    baud: int = 115200

    # reconnect_period: 断线重连间隔 (秒)
    # 串口断开后, 每隔这么多秒尝试重新连接一次
    # 设为 1.0 秒: 既不频繁 (浪费 CPU), 也足够快 (1 秒内恢复)
    reconnect_period: float = 1.0

    # startup_delay: 串口打开后的稳定延时 (秒)
    # 某些 USB 转串口芯片 (如 CH340) 打开后需要短暂延时通信才稳定
    startup_delay: float = 0.2

    # startup_stop_frames: 启动时发送的停车帧数量
    # 节点启动后先连续发送 N 帧零速度指令
    # 清除 STM32 底盘可能保留的上次会话的旧运动命令
    startup_stop_frames: int = 5

    # =========================================================================
    # ROS 话题名称和 TF 坐标系名称
    # =========================================================================
    # cmd_vel_topic: 订阅的速度指令话题名
    # 默认 "/cmd_vel" 是 ROS 导航栈的标准话题名
    cmd_vel_topic: str = "/cmd_vel"

    # odom_topic: 发布的里程计话题名
    # 默认 "/odom" 是 ROS 导航栈的标准话题名
    odom_topic: str = "/odom"

    # imu_topic: 发布的 IMU 数据话题名
    # 默认 "/imu/data_raw" 表示这是 "原始" (raw) IMU 数据
    imu_topic: str = "/imu/data_raw"

    # odom_frame: 里程计坐标系的 TF 名称
    # ROS 导航栈期望这个名称固定为 "odom"
    odom_frame: str = "odom"
    ...
    # =========================================================================
    # tx_rate: 速度指令发送频率 (Hz)
    # 50 Hz 意味着每 20ms 发送一次速度指令
    # 高于底盘响应频率没有意义, 低于控制要求则响应延迟
    tx_rate: float = 50.0

    # cmd_timeout: 速度指令看门狗超时 (秒)
    # 如果超过这个时间没有收到新的 /cmd_vel 消息,
    # 节点自动发送零速度指令 (停车)
    # 这是重要的安全机制: 导航栈崩溃时底盘不会一直按最后的速度运动
    cmd_timeout: float = 0.3

    # feedback_timeout: 底盘反馈超时 (秒)
    # 用于状态显示: 超过这个时间没有收到反馈帧, 状态显示为 "超时"
    # 不代表底盘已断线 (断线检测由串口异常触发)
    feedback_timeout: float = 0.5

    # max_linear_x: X 轴线速度的最大绝对值 (m/s)
    # 底盘前进/后退的最大速度
    max_linear_x: float = 0.5

    # max_linear_y: Y 轴线速度的最大绝对值 (m/s)
    # 底盘横向平移的最大速度
    # 麦克纳姆底盘特有, 普通差分驱动底盘无法横向移动
    max_linear_y: float = 0.5

    # max_angular_z: Z 轴角速度的最大绝对值 (rad/s)
    # 底盘原地旋转的最大角速度
    # 1.2 rad/s ≈ 69 度/秒
