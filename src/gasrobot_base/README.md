...
├── protocol.py          # 9/22 字节协议、校验和与流式拆帧
...
| 19 | `yaw` | 无符号单字节整周航向 |
| 20 | 校验和 | 字节 1～19 的无符号累加低八位 |
| 21 | 帧尾 | `0x7D` |

`yaw` 将一周均分为 256 份，`0～255` 对应 `0～360°`，每计数为
`1.40625°`。ROS 解码后将角度归一化到 `[-π, π]`。单字节航向的最大量化误差
约为 `0.703°`；若后续实验要求更高的姿态分辨率，应将协议升级为双字节航向，
而不是在 ROS 端重新积分角速度。

协议层负责字节序、单位和坐标方向差异，ROS 节点只接触已解码数据。22 字节
反馈与旧的 21 字节反馈不兼容，必须同步更新并烧录 STM32 固件。
...
测试覆盖控制帧编码、22 字节反馈帧校验、单字节整周航向解码、串口数据分片与
